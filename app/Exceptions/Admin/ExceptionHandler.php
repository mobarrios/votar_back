    ...
    public function createResponse($exception)
    {
        ...
        if (!$this->debug){
            if($exception->getStatusCode() === 403){
                return redirect()->back()->withErrors('No tenés permiso para acceder a esa sección');
            }
            ...
            if($exception->getStatusCode() != 403 && $exception->getStatusCode() != 404){
                $data = [
                    ...
                    "method" => URL::getRequest()->getMethod(),
                    "code" => $exception->getStatusCode(),
                    "message" => $exception->getMessage(),
                    "trace" => collect($exception->getTrace())->take(3)
                ];

                Mail::queue('emailBugs', ["data" => $data] , function($message)
                {
                    $message->from("reporte@bugs.com");
                    $message->to(env('EMAIL_BUGS'))->subject('Reporte de bug')
                        ->replyTo(env('EMAIL_BUGS'), 'Soporte');
                });
            }

            return Response::create(view('errors.err'));
        }

        //esto es lo que siempre hace en testing
